...
const list = ref([
  { value: 'label_1', label: '首页', icon: 'home', num: 0 },
  { value: 'label_3', label: '聊天', icon: 'chat', num: 8 },
  { value: 'label_4', label: '我的', icon: 'user', num: 0 },
])
...
<template>
  <div class="layout-wrapper p-4">
    ...
    <t-tab-bar v-model="value" theme="tag" :split="false">
      <t-tab-bar-item v-for="item in list" :key="item.value" :value="item.value" :badge-props="{ count: item.num, offset: [-8, 3] }">
        {{ item.label }}
        <template #icon>
